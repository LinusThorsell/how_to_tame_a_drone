...
function Gate({ position, active, passed }) {
  const color = active ? '#ff0096' : passed ? '#6ee7a8' : '#58305f';
  const intensity = active ? 3.2 : passed ? 1.2 : 0.15;
  return (
    <group position={[position.x, position.y, position.z]}>
      <mesh position={[-2.5, 0, 0]} castShadow><boxGeometry args={[0.16, 4.8, 0.18]} /><meshStandardMaterial color={color} emissive={color} emissiveIntensity={intensity} /></mesh>
      <mesh position={[2.5, 0, 0]} castShadow><boxGeometry args={[0.16, 4.8, 0.18]} /><meshStandardMaterial color={color} emissive={color} emissiveIntensity={intensity} /></mesh>
      <mesh position={[0, 2.4, 0]} castShadow><boxGeometry args={[5.15, 0.16, 0.18]} /><meshStandardMaterial color={color} emissive={color} emissiveIntensity={intensity} /></mesh>
      <mesh position={[0, -2.4, 0]} castShadow><boxGeometry args={[5.15, 0.16, 0.18]} /><meshStandardMaterial color={color} emissive={color} emissiveIntensity={intensity} /></mesh>
      {active && <pointLight color="#ff0096" intensity={16} distance={11} decay={2} />}
    </group>
  );
}

function RangeObjects() {
  const markers = useMemo(() => Array.from({ length: 30 }, (_, index) => ({
...
function Ground() {
  return (
    <RigidBody type="fixed" colliders={false} position={[0, -0.08, 70]}>
      <CuboidCollider args={[110, 0.08, 110]} friction={0.85} restitution={0.05} />
      ...
      <gridHelper args={[220, 110, '#a02a9e', '#4a184f']} position={[0, 0.09, 0]} />
    </RigidBody>
  );
}
...
function Simulator({ launched, controller, gains, resetSignal, onTelemetry, onCheckpoint }) {
  ...
  useEffect(() => { resetPhysics(); }, [resetSignal, launched, resetPhysics]);
  ...
  useBeforePhysicsStep((world) => {
    ...
    const target = CHECKPOINTS[flight.checkpoint];
    if (target && Math.hypot(target.x - position.x, target.y - position.y, target.z - position.z) < 3.1) {
      flight.checkpoint += 1;
      onCheckpointRef.current?.(flight.checkpoint);
    }
  });
  ...
  return (
    <>
      ...
      {CHECKPOINTS.map((position, index) => <Gate key={index} position={position} active={index === checkpoint} passed={index < checkpoint} />)}
      <RangeObjects />
    </>
  );
}
...
export default function FlightScene(props) {
  return (
    <Canvas
      ...
      shadows
      dpr={[1, 1.6]}
      camera={{ position: [0, 4.6, -5], fov: 55, near: 0.1, far: 240 }}
      gl={{ antialias: true, alpha: false, powerPreference: 'high-performance' }}
    >
      <color attach="background" args={['#160313']} />
      <fog attach="fog" args={['#160313', 35, 125]} />
